...
const CalendarPage = () => {
  ...
  const filteredSessions = sessions.filter(session => isSameDay(session.date, selectedDate));
  const weekStart = startOfWeek(selectedDate, { weekStartsOn: 1 });
  const weekDays = Array.from({ length: 7 }, (_, i) => addDays(weekStart, i));

  const getSessionsForDay = (date: Date) => sessions.filter(session => isSameDay(session.date, date));
